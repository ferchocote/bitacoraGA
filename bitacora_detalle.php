...
if (!$proceso) { echo '<p>Proceso no encontrado.</p>'; return; }
  ...
  <div class="modal-container">
    ...
    <form method="post" action="?view=editar_proceso&id=<?= esc_attr($proceso->Id) ?>" class="popup-grid-4">
      ...
      <div class="form-group"><label for="IdEmpresa">Cliente:</label>
        <select id="IdEmpresa" name="IdEmpresa"><?php foreach($clientes as $c): ?><option value="<?= $c->Id ?>" <?= selected($proceso->IdEmpresa,$c->Id,false) ?>><?= esc_html($c->RazonSocial) ?></option><?php endforeach; ?></select></div>
      ...
      <div class="form-group"><label for="IdEstadoProceso">Estado:</label>
        <select id="IdEstadoProceso" name="IdEstadoProceso"><?php foreach($estadosList as $st): ?><option value="<?= $st->Id ?>" <?= selected($proceso->IdEstadoProceso,$st->Id,false) ?>><?= esc_html($st->Descripcion) ?></option><?php endforeach; ?></select></div>
      ...
      <div class="form-group"><label for="ETA">ETA:</label>
        ...
        <!-- Resto de campos -->
      <?php foreach([
        'DiasLibres'=>'DiasLibres','DigitacionRevision'=>'DigitacionRevision',
        'Aduana'=>'Aduana','Producto'=>'Producto','NumeroBL'=>'NumeroBL',
        'Contenedor'=>'Contenedor','Manifiesto'=>'Manifiesto','Pies'=>'Pies',
        'Bulto'=>'Bulto','PesoBruto'=>'PesoBruto','Bandera'=>'Bandera'
      ] as $field => $prop): ?>
      ...
      <div class="form-group"><label for="Puerto">Puerto:</label>
        <input type="text" id="Puerto" name="Puerto" value="<?= esc_attr($detalle ? $detalle->Puerto : '') ?>"></div>
      <div class="form-group"><label for="Observaciones">Observaciones:</label>
        <input type="text" id="Observaciones" name="Observaciones" value="<?= esc_attr($detalle ? $detalle->Observaciones : '') ?>"></div>
