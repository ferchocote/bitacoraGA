...
function validate_jwt($token, $secret) {
    ...
    $signature_check = hash_hmac('sha256', "$header64.$payload64", $secret, true);
    $signature_check_encoded = rtrim(strtr(base64_encode($signature_check), '+/', '-_'), '=');

    if (!hash_equals((string)$signature64, (string)$signature_check_encoded)) {
        return false;
    }
...
$usuario = $wpdb->get_row("SELECT u.*, r.Nombre AS rol_nombre, r.Codigo AS rol_codigo, u.IdAliado, ge.Nombre AS grupo_nombre
        FROM wp_users u
        ...
        <div class="sidebar" id="sidebar">
            ...
            <div>
                ...
                <a href="<?php echo wp_logout_url(home_url('/wp-content/bitacoras/login.php')); ?>">
                    ...
                    <span class="menu-option-span">Salir</span>
                </a>
            </div>
        </div>
